...
export default function MovieWatchClient({ subjectId, initialSeason = 1, initialEpisode = 1, isSeries = false }) {
  ...
  useEffect(() => {
    fetchVideoData();
  }, [subjectId, currentSeason, currentEpisode]);

  const fetchStreamUrl = async (directUrl) => {
  ...
  const ensureSubtitleBlob = async (caption) => {
  ...
  // Ensure selected subtitle is shown when user switches language
  useEffect(() => {
